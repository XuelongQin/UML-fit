...
if [ ! -d logs_parSub_explCon ]; then mkdir logs_parSub_explCon; fi
...
Log         = logs_parSub_explCon/sub_\$(ClusterId).log
Output      = logs_parSub_explCon/simfit_recoMC_fullAngularMass_toybkg_\$INT(nsamp)_\$INT(bin).out
Error       = logs_parSub_explCon/simfit_recoMC_fullAngularMass_toybkg_\$INT(nsamp)_\$INT(bin).err
...
echo 'Queue 60'>>temp_sub_simfit_recoMC_fullAngularMass_toybkg_oneBin.sub

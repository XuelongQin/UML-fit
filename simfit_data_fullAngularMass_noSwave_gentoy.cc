...
double mFJpsi[3] = {0.977,1.105,1.067};
//this code is used to calculate sys.error of mistag fraction
...
void simfit_data_fullAngularMass_SwaveBin(int q2Bin, int parity, bool multiSample, uint nSample, bool localFiles, bool plot, bool save, std::vector<int> years)
{
  ...
  RooWorkspace* wksp = new RooWorkspace(((multiSample?"wsMulti_":"ws_")+shortString+Form("_s%i_pow%.0f",nSample,power)).c_str(),
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    cout << "going to read file" << endl;
	 //string filename_data = Form("./simFitResults4d/simFitResult_recoMC_fullAngularMass_toybkg201620172018_dataStat-0-0_b%i_toy.root", q2Bin);
    ...
    //retrieveWorkspace( filename_data, wsp, Form("wsMulti_b%ip1_s1_pow1", q2Bin ));
...
//    else
    ...
    for (uint itoy = 0; itoy <= lastSample-firstSample; itoy++){
      ...
      RooDataSet *toy_bkg = bkg_pdf->generate(observables,nbkg_togen);
...
//    cout << fraction << " +/- " << fM_sigmas[years[iy]][q2Bin] << " ---> 1 +/- " << frac_sigma << endl;                                    
    ...
    wksp->loadSnapshot(Form("gen_bkg_pdf_%i",years[iy]));
    //RooArgSet* bkg_ang_params = (RooArgSet*)bkg_ang_pdf->getParameters(observables);
    auto iter = bkg_ang_params->createIterator();
    RooRealVar* ivar =  (RooRealVar*)iter->Next();
    while (ivar) {
    ...
    // read mass pdf for background
    //RooRealVar* slope       = new RooRealVar    (Form("slope^{%i}",years[iy]),  Form("slope^{%i}",years[iy]) , -5., -10., wksp);
    // RooRealVar* slope       = new RooRealVar    (Form("slope^{%i}",years[iy]),  Form("slope^{%i}",years[iy]) , wsp_sb[iy]->var("slope")->getVal(), -10., 0.);
  ...
  TFile* fout = 0;
  if (save) fout = new TFile(("simFitResults4d/mFsys/simFitResult_data_fullAngularMass_Swave_gentoy_ctmF_test_" + all_years + stat + Form("_b%i.root", q2Bin)).c_str(),"RECREATE");
  ...
  */
  ...
  c[confIndex]->SaveAs( ("plotSimFit4d_d/mFsys/simFitResult_data_fullAngularMass_Swave_gentoy_ctmF_test_" + plotString +  ".pdf").c_str() );

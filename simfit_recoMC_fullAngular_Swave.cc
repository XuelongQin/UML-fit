...
void simfit_recoMC_fullAngular_SwaveBin(int q2Bin, int parity, bool multiSample, uint nSample, bool localFiles, bool plot, bool save, std::vector<int> years)
{
...
//   RooArgSet reco_vars (*ctK, *ctL, *phi, *rand);
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    string filename2 = Form((parity==0 ? "integrSwave_b%i_ev_%i.root" : "integrSwave_b%i_od_%i.root"),q2Bin,years[iy]);
    if (!localFiles) filename  = Form("/eos/cms/store/user/fiorendi/p5prime/effKDE/%i/lmnr/newphi/",years[iy]) + filename;
    if (!localFiles) filename2 = Form("/eos/user/a/aboletti/BdToKstarMuMu/eff-KDE-Swave/%i/lmnr/",years[iy]) + filename2;
    ...
    auto fin_intSwave = new TFile( filename2.c_str(), "READ" );
    if ( !fin_intSwave || !fin_intSwave->IsOpen() ) {
      cout<<"File not found: "<<filename2<<endl;
      return;
    }
    auto intCHistSwave = (TH1D*)fin_intSwave->Get(intCHistString.c_str());
    auto intWHistSwave = (TH1D*)fin_intSwave->Get(intWHistString.c_str());
    ...
    if ( !intCHist[iy] || intCHist[iy]->IsZombie() ||
	 !intWHist[iy] || intWHist[iy]->IsZombie() ||
	 !intCHistSwave || intCHistSwave->IsZombie() ||
	 !intWHistSwave || intWHistSwave->IsZombie() ) {
      cout << "Integral histogram " << intCHistString <<" or " << intWHistString << " not found in file: "<< filename << " or " << filename2 << endl << "Abort" << endl;
      return;
    } else if ( strcmp( intCHist[iy]->GetTitle(), effCHist[iy]->GetTitle() ) ||
		strcmp( intWHist[iy]->GetTitle(), effWHist[iy]->GetTitle() ) ||
		strcmp( intCHistSwave->GetTitle(), effCHist[iy]->GetTitle() ) ||
		strcmp( intWHistSwave->GetTitle(), effWHist[iy]->GetTitle() )) {
    // if the eff_config tag is different between efficiency and precomputed-integral means that they are inconsistent
      ...
      cout << "S-wave int (CT) conf: " << intCHistSwave->GetTitle()<<endl;
      cout << "Efficiency (WT) conf: " << effWHist[iy]->GetTitle() <<endl;
      cout << "Integral (WT) conf: "   << intWHist[iy]->GetTitle() <<endl;
      cout << "S-wave int (WT) conf: " << intWHistSwave->GetTitle()<<endl;
      cout << "Abort"<<endl;
      return;
    } 
    else {
      ...
      for (int i=1; i<=intCHistSwave->GetNbinsX(); ++i) {
        intCVec[iy].push_back(intCHistSwave->GetBinContent(i));
      }
      for (int i=1; i<=intWHist[iy]->GetNbinsX(); ++i) {
        intWVec[iy].push_back(intWHist[iy]->GetBinContent(i));
      }
      for (int i=1; i<=intWHistSwave->GetNbinsX(); ++i) {
        intWVec[iy].push_back(intWHistSwave->GetBinContent(i));
      }
    }
    cout<<"Integral vectors filled with "<<intCVec[iy].size()<<" (CT) and "<<intWVec[iy].size()<<" (WT) entries"<<endl;
  ...
  TFile* fout;
  if (save) fout = new TFile(("simFitResults/simFitResult_recoMC_fullAngular_Swave_" + all_years + stat + Form("_b%i.root", q2Bin)).c_str(),"RECREATE");
...
*/
  ...
  c[confIndex]->SaveAs( ("plotSimFit_d/simFitResult_recoMC_fullAngular_Swave_" + shortString + "_" + all_years + stat + ".pdf").c_str() );

}


void simfit_recoMC_fullAngular_SwaveBin1(int q2Bin, int parity, bool multiSample, uint nSample, bool localFiles, bool plot, bool save, std::vector<int> years)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      simfit_recoMC_fullAngular_SwaveBin(q2Bin, parity, multiSample, nSample, localFiles, plot, save, years);
  else
    simfit_recoMC_fullAngular_SwaveBin(q2Bin, parity, multiSample, nSample, localFiles, plot, save, years);
}
...
int main(int argc, char** argv)
{
  ...
  if ( q2Bin==-1 )
    for (q2Bin=0; q2Bin<nBins; ++q2Bin)
      simfit_recoMC_fullAngular_SwaveBin1(q2Bin, parity, multiSample, nSample, localFiles, plot, save, years);
  else
    simfit_recoMC_fullAngular_SwaveBin1(q2Bin, parity, multiSample, nSample, localFiles, plot, save, years);

...
#include "utils.h"
#include "PdfSigRTMass1.h"
#include "PdfSigWTMass1.h"
...
//    cout << fraction << " +/- " << fM_sigmas[years[iy]][q2Bin] << " ---> 1 +/- " << frac_sigma << endl;                                    
  ...
  TFile* fout = 0;
  if (save) fout = new TFile(("simFitResults4d/simFitResult_data_fullAngularMass_Swave_rmctwt_width_" + all_years + stat + Form("_b%i.root", q2Bin)).c_str(),"RECREATE");
  ...
  */
  ...
  c[confIndex]->SaveAs( ("plotSimFit4d_d/simFitResult_data_fullAngularMass_Swave_rmctwt_width_" + plotString +  ".pdf").c_str() );

...
double power = 1.0;
//This code is used to evaluate the sys.unc for mistag fraction, with cocktail MC
void simfit_data_fullAngularMass_SwaveBin(int q2Bin, int parity, bool multiSample, uint nSample, uint q2stat, bool localFiles, bool plot, int save, std::vector<int> years, int XGBv, bool usefreeMF)
{
  ...
  string MFcentralString = "_withoutfreeMF_";
  std::vector<double> MFcentral;
  MFcentral = {1.0,1.0,1.0};
  if (usefreeMF){
    MFcentral={1.0684,1.0501,1.0519};
    MFcentralString = "_withfreeMF_";
  }
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    // string filename_data = Form("recoMCDataset_b%i_%i.root", q2Bin, years[iy]);
    if (!localFiles) filename_data = "/afs/cern.ch/user/x/xuqin/work/B0KstMuMu/sys/UML-fit/dataset/cocktail_XGBv8/" + filename_data;
...
//     c_vars.add(*deltaPeaks);       c_pdfs.add(*c_deltaPeaks[iy]);
    ...
    RooGaussian* c_fm = new RooGaussian(Form("c_fm^{%i}",years[iy]) , "c_fm" , *mFrac,  
                                        RooConst(MFcentral[iy]) , 
                                        RooConst(frac_sigma)
                                        );
    cout << fraction << " +/- " << fM_sigmas[years[iy]][q2Bin] << " ---> " << MFcentral[iy]<<" +/- " << frac_sigma << endl;                                    
    c_vars.add(*mFrac); 
  ...
  TFile* fout = 0;
  if (save>0) fout = new TFile(("simFitResults4d/simFitResult_cocktaiMC_fullAngularMass_Swave_" + MFcentralString + all_years + stat + Form("_b%i%s.root", q2Bin, XGBstr.c_str())).c_str(),"RECREATE");
  ...
  for (uint is = firstSample; is <= lastSample; is++) {
    ...
    *params = *savedParams ;
	...
	string plotname = "plotSimFit4d_d/simFitResult_cocktailMC_fullAngularMass_Swave_" + MFcentralString +plotString + ".pdf";
...
void simfit_data_fullAngularMass_SwaveBin1(int q2Bin, int parity, bool multiSample, uint nSample, uint q2stat, bool localFiles, bool plot, int save, std::vector<int> years, int XGBv, bool usefreeMF)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      simfit_data_fullAngularMass_SwaveBin(q2Bin, parity, multiSample, nSample, q2stat, localFiles, plot, save, years, XGBv,usefreeMF);
  else
    simfit_data_fullAngularMass_SwaveBin(q2Bin, parity, multiSample, nSample, q2stat, localFiles, plot, save, years, XGBv,usefreeMF);
}
...
int main(int argc, char** argv)
{
  ...
  bool usefreeMF = true;
  if (argc>10 && atoi(argv[10]) == 0) usefreeMF=false;
  ...
  */
  ...
  if ( q2Bin==-1 )
    for (q2Bin=0; q2Bin<nBins; ++q2Bin)
      simfit_data_fullAngularMass_SwaveBin1(q2Bin, parity, multiSample, nSample, q2stat, localFiles, plot, save, years, XGBv,usefreeMF);
  else
    simfit_data_fullAngularMass_SwaveBin1(q2Bin, parity, multiSample, nSample, q2stat, localFiles, plot, save, years, XGBv,usefreeMF);

...
void simfit_recoMC_fullAngularMass_toyeffBin(int q2Bin, int parity, uint nSample, bool localFiles, bool plot, bool save, std::vector<int> years)
{
  ...
  uint firstSample = ( nSample==0 ) ? 0 : nSample-1;
  
  std::vector<TFile*> fin_eff(years.size());
  std::vector<RooWorkspace*> wsp, wsp_mcmass;
  std::vector<std::vector<RooDataSet*>> data;
  std::vector<RooAbsReal*> effC(years.size());
  std::vector<RooAbsReal*> effW(years.size());
  std::vector<TH3D*> effCHist(years.size());
  std::vector<TH3D*> effWHist(years.size());
  std::vector<TH1D*> intCHist(years.size());
  std::vector<TH1D*> intWHist(years.size());
  ...
  std::vector<RooAbsPdf*> PDF_sig_ang_mass(years.size());
  std::vector<RooAbsPdf*> PDF_sig_ang_mass_penalty(years.size());
  std::vector<RooGaussian*> c_deltaPeaks, c_fm;
  RooArgSet c_vars_rt, c_pdfs_rt;
  RooArgSet c_vars_wt, c_pdfs_wt;
  RooArgSet c_vars;
  std::vector<RooProdPdf*> c_dcb_rt(years.size());
  std::vector<RooProdPdf*> c_dcb_wt(years.size());
  std::vector<RooRealVar*> mFrac(years.size());
  ...
  RooRealVar* phi = new RooRealVar("phi", "phi", -1*TMath::Pi(), TMath::Pi()  );
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    all_years += year;
    isample.clear(); isample.assign( Form("%i",firstSample) );
    sample.defineType(("data"+year+"_subs"+isample).c_str());
  } 

  // Construct a simultaneous pdf using category sample as index
  RooSimultaneous* simPdf;
  RooSimultaneous* simPdf_penalty;
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    data.push_back( createDataset( nSample,  firstSample,  firstSample, wsp[iy],  
    ...
    c_dcb_rt[iy] = new RooProdPdf(("c_dcb_rt_"+year).c_str(), ("c_dcb_rt_"+year).c_str(), constr_rt_list );
    ...
    c_dcb_wt[iy] = new RooProdPdf(("c_dcb_wt_"+year).c_str(), ("c_dcb_wt_"+year).c_str(), constr_wt_list );
    c_vars.add(c_vars_wt);

    mFrac[iy] = new RooRealVar(Form("f_{M}^{%i}",years[iy]),"mistag fraction",1, 0.5, 1.5);
    ...
    c_fm.push_back( new RooGaussian(Form("c_fm^{%i}",years[iy]) , "c_fm" , *mFrac[iy],  
				    RooConst(1.) , 
				    RooConst(frac_sigma)
				    ));
//     cout << fraction << " +/- " << fM_sigmas[years[iy]][q2Bin] << " ---> << "1 +/- " << frac_sigma << endl;                                    
    ...
    // import KDE efficiency partial integral histograms from original efficiency
    ...
    filename = "/eos/user/a/aboletti/BdToKstarMuMu/eff-KDE-Swave/files/" + filename;
    fin_eff[iy] = new TFile( filename.c_str(), "READ" );
    if ( !fin_eff[iy] || !fin_eff[iy]->IsOpen() ) {
      cout<<"File not found: "<<filename<<endl;
      return;   
    }
    // import precomputed integrals and fill a std::vector
    intCHist[iy] = (TH1D*)fin_eff[iy]->Get(intCHistString.c_str());
    intWHist[iy] = (TH1D*)fin_eff[iy]->Get(intWHistString.c_str());
    intCVec[iy] = vector<double> (0);
    intWVec[iy] = vector<double> (0);
  ...
  RooArgSet *params = 0;
  RooArgSet* savedParams = 0;

  if (nSample>0)   stat = stat + Form("-%i",firstSample);
  string foutName = "simFitResults4d/simFitResult_recoMC_fullAngularMass_toyeff" + all_years + stat + Form("_b%i.root", q2Bin);
  ...
  for (uint iToy = 0; iToy<10; ++iToy) {

    simPdf = new RooSimultaneous(Form("simPdf%i",iToy), "simultaneous pdf", sample);
    simPdf_penalty = new RooSimultaneous(Form("simPdf_penalty%i",iToy), "simultaneous pdf with penalty term", sample);

    for (unsigned int iy = 0; iy < years.size(); iy++) {
      year.clear(); year.assign(Form("%i",years[iy]));

      // import KDE efficiency histograms and partial integral histograms
      string filename = Form((parity==0 ? "KDEeff_b%i_ev_%i_toy%i.root" : "KDEeff_b%i_od_%i_toy%i.root"),q2Bin,years[iy],iToy);
      // if (!localFiles) filename = Form("/eos/cms/store/user/fiorendi/p5prime/effKDE/%i/lmnr/newphi/",years[iy]) + filename;
      filename = "/eos/user/a/aboletti/BdToKstarMuMu/eff-KDE-Swave/files/" + filename;
      fin_eff[iy] = new TFile( filename.c_str(), "READ" );
      if ( !fin_eff[iy] || !fin_eff[iy]->IsOpen() ) {
	...
	return;   
      ...
      effCHist[iy] = (TH3D*)fin_eff[iy]->Get(effCString.c_str());
      effWHist[iy] = (TH3D*)fin_eff[iy]->Get(effWString.c_str());
	...
	return;
      ...
      effC[iy] = new RooHistFunc(("effC_"+shortString+"_"+year).c_str(),
				 ("effC"+year).c_str() ,
				 vars,
				 *effCData,
				 1);
      effW[iy] = new RooHistFunc(("effW_"+shortString+"_"+year).c_str(),
				 ("effW"+year).c_str() ,
				 vars,
				 *effWData,
				 1);

      if ( strcmp( intCHist[iy]->GetTitle(), effCHist[iy]->GetTitle() ) || strcmp( intWHist[iy]->GetTitle(), effWHist[iy]->GetTitle() )) {
	...
	return;
      ...
      PDF_sig_ang_mass[iy] = new PdfSigAngMass( ("PDF_sig_ang_mass_"+shortString+"_"+year).c_str(),
      ...
      PDF_sig_ang_mass_penalty[iy] = new PdfSigAngMass( ( "PDF_sig_ang_mass_penalty_"+shortString+"_"+year).c_str(),
...
void simfit_recoMC_fullAngularMass_toyeffBin1(int q2Bin, int parity, uint nSample, bool localFiles, bool plot, bool save, std::vector<int> years)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      simfit_recoMC_fullAngularMass_toyeffBin(q2Bin, parity, nSample, localFiles, plot, save, years);
  else
    simfit_recoMC_fullAngularMass_toyeffBin(q2Bin, parity, nSample, localFiles, plot, save, years);
}
...
//   cout << years[2] << endl;
  ...
  if ( q2Bin==-1 )
    for (q2Bin=0; q2Bin<nBins; ++q2Bin)
      simfit_recoMC_fullAngularMass_toyeffBin1(q2Bin, parity, nSample, localFiles, plot, save, years);
  else
    simfit_recoMC_fullAngularMass_toyeffBin1(q2Bin, parity, nSample, localFiles, plot, save, years);

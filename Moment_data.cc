...
tuple<RooDataSet*,RooDataSet*,RooDataSet*> splot(RooWorkspace *ws_pdf ,int q2Bin, int year, int isample){
    ...
    //RooRealVar *totnum = ws_pdf->var(Form("totnumb%ip1_%i",q2Bin,year));
    //totnum->setConstant(true);
    //RooFormulaVar*nbkg_fm = new  RooFormulaVar(Form("nbkg_fmb%ip1_%i",q2Bin,year), "@0 - @1", RooArgList(*totnum, *nsig));
    ...
    //RooRealVar *nbkg = (RooRealVar*) nbkg_fm;
...
void data_MOM(int q2Bin,std::vector<int> years){
    ...
    string outname = "./Momresults/data/data_noaddpdf_" ;
    ...
    // create exponential mass pdf for background
       ...
       // RooRealVar *totnum = new RooRealVar(("totnum"+shortString+"_"+year).c_str(),"totalnum", fullentries);
        //totnum->setConstant(true);
        //RooFormulaVar*nbkg_fm = new  RooFormulaVar(("nbkg_fm"+shortString+"_"+year).c_str(), "@0 - @1", RooArgList(*totnum, *nsig));
    ...
    for (uint is = firstSample; is <= lastSample; is++) {
        ...
        c[confIndex]->SaveAs( ("plotMom_Fit/data/simFitResult_data_fullMass_noaddpdf" + plotString +  ".pdf").c_str() );
    }
    fout->cd();
    ws_pars->Write();
    momResultsTree->Write();

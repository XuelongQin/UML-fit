...
void simfit_recoMC_fullAngularMassBin(int q2Bin, int parity, int nEff, uint nSample, bool localFiles, bool plot, int save, std::vector<int> years)
{
  ...
  string year = ""; 
  string isample = ""; 
  string stat = Form("_dataStat-%i",nSample);
  ...
  RooWorkspace* wksp = new RooWorkspace(("ws_"+shortString+Form("_s%i_pow%.1f",nSample+1,power)).c_str(),
					"Workspace with RECO subsample fit result");
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    filename_toyeff = "/eos/user/a/aboletti/BdToKstarMuMu/eff-KDE-Swave/files/" + filename_toyeff;
    ...
    effCHist.push_back( (TH3D*)fin_toyeff->Get(effCString.c_str()));
    effWHist.push_back( (TH3D*)fin_toyeff->Get(effWString.c_str()));
    if ( !effCHist[iy] || effCHist[iy]->IsZombie() || !effWHist[iy] || effWHist[iy]->IsZombie() ) {
      cout<<"Efficiency histogram "<< effCString <<" or " << effWString << " not found in file: "<< filename_toyeff <<endl;
      return;
    }
    ...
    // import KDE partial integral histograms from default efficiency
...
//     c_vars.add(*deltaPeaks);       c_pdfs.add(*c_deltaPeaks[iy]);
  ...
  TFile* fout = new TFile(("simFitResults4d/simFitResult_recoMC_fullAngularMass_toybkg" + all_years + stat + Form("_b%i_toyeff%i.root", q2Bin, nEff)).c_str(),"RECREATE");
...

  {
    ...
    cout<<"Fitting subsample "<<is+1<<" with "<<combData->numEntries()<<" entries"<<endl;
...
//   }
// 
  string plotString = shortString + "_" + all_years + Form("_s%i_toyeff%i",nSample+1,nEff);
// 
//   cnll->Update();
...
//   cPen->SaveAs( ("plotSimFit4d_d/recoPenTerm_" + plotString + "_toybkg.pdf").c_str() );
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    for (unsigned int fr = 0; fr < frames.size(); fr++){
        ...
        simPdf->plotOn(frames[fr], Slice(sample, ("data"+year+Form("_subs%d",nSample)).c_str()), 
...
void simfit_recoMC_fullAngularMassBin1(int q2Bin, int parity, int nEff, uint nSample, bool localFiles, bool plot, int save, std::vector<int> years)
{
  if ( parity==-1 )
    for (parity=0; parity<2; ++parity)
      simfit_recoMC_fullAngularMassBin(q2Bin, parity, nEff, nSample, localFiles, plot, save, years);
  else
    simfit_recoMC_fullAngularMassBin(q2Bin, parity, nEff, nSample, localFiles, plot, save, years);
}
...
//   cout << years[2] << endl;
  ...
  if ( q2Bin==-1 )
    for (q2Bin=0; q2Bin<nBins; ++q2Bin)
      simfit_recoMC_fullAngularMassBin1(q2Bin, parity, nEff, nSample, localFiles, plot, save, years);
  else
    simfit_recoMC_fullAngularMassBin1(q2Bin, parity, nEff, nSample, localFiles, plot, save, years);

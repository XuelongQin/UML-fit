...
void simfit_recoMC_fullAngularMassBin(int q2Bin, int parity, bool multiSample, uint nSample, int XGBv, bool localFiles, bool plot, int save, std::vector<int> years)
{
  ...
  std::vector<RooWorkspace*> wsp, wsp_mcmass, wsp_sb, wsp_sb_ref, wsp_toy_bkg;
  ...
  RooArgSet reco_vars (*ctK, *ctL, *phi, *mass, *rand, *wei);
  ...
  for (unsigned int iy = 0; iy < years.size(); iy++) {
    ...
    string filename_sb_local = Form("savesb_%i_b%i.root", years[iy], q2Bin );
    
    string filename_toy_bkg = Form("toy_bkg_%i_b%i.root",years[iy], q2Bin );
    RooDataSet *toy_bkg =0;
    uint itoy = lastSample;
    TFile test( filename_toy_bkg.c_str());
    if (test.IsZombie() ){
     ...
     wsp_sb_ref[iy]->ls();
     wsp_sb_ref[iy]->pdf(Form("BernSideBand_bin%i_%i", q2Bin, years[iy]));

     if(!wsp_sb_ref[iy]){
      cout << Form("Error! Not found BernSideBand_bin%i_%i in %s", q2Bin, years[iy],filename_sb_ref.c_str())  << endl;
     }
     RooBernsteinSideband* bkg_ang_pdf_ref = (RooBernsteinSideband*) wsp_sb_ref[iy]->pdf(Form("BernSideBand_bin%i_%i", q2Bin, years[iy]));
    
    

    // read mass pdf for background
     RooRealVar* slope_ref	 = new RooRealVar    (Form("slope_ref^{%i}",years[iy]),  Form("slope_ref^{%i}",years[iy]) , wsp_sb_ref[iy]->var("slope")->getVal(), -10., 0.);
     RooExponential* bkg_exp_ref = new RooExponential(Form("bkg_exp_ref_%i",years[iy]),  Form("bkg_exp_ref_%i",years[iy]) ,  *slope_ref,   *mass  );
     RooArgSet*  bkg_ang_params_ref = (RooArgSet*) bkg_ang_pdf_ref->getParameters(observables);
     auto iter_ref = bkg_ang_params_ref->createIterator();
     RooRealVar* ivar_ref =  (RooRealVar*)iter_ref->Next();
     while (ivar_ref) {
       ivar_ref->setConstant(true);
       ivar_ref = (RooRealVar*) iter_ref->Next();
     }
     cout << Form("exponential slope_ref for %f", slope_ref->getVal())  << endl;


     // retrieve sideband range from input file
     float max_lsb = wsp_sb_ref[iy]->var(Form("max_sbl_bin%i_%i", q2Bin, years[iy]))->getVal();
     float min_rsb = wsp_sb_ref[iy]->var(Form("min_sbr_bin%i_%i", q2Bin, years[iy]))->getVal();
     ...
     RooProdPdf* bkg_pdf_ref = new RooProdPdf(Form("bkg_pdf_ref_%i",years[iy]), Form("bkg_pdf_ref_%i",years[iy]), RooArgList(*bkg_ang_pdf_ref,*bkg_exp_ref));
     wksp->import(*bkg_ang_pdf_ref, RecycleConflictNodes());
     wksp->import(*bkg_pdf_ref, RecycleConflictNodes());
 
//     RooArgSet*  bkg_params_ref       = (RooArgSet*)bkg_pdf_ref->getParameters(observables);
//     RooArgSet*  saved_bkg_params = (RooArgSet*)bkg_params->snapshot() ;
//     wksp->saveSnapshot(Form("gen_bkg_pdf_%i",years[iy]), *bkg_params_ref, true) ;
...
//     wksp->defineSet(Form("bkg_params_ref_%i",years[iy]),     *bkg_params_ref, true);
    
     RooAbsPdf::GenSpec* genSpec = bkg_pdf_ref->prepareMultiGen( observables, NumEvents(gen_nevt.Poisson(nbkg_togen)));
...
//    for (uint itoy = 0; itoy <= lastSample-firstSample; itoy++){
     ...
     toy_bkg = bkg_pdf_ref->generate(*genSpec) ;
     toy_bkg->SetName(Form("data_toy_bkg_%i_b%i",years[iy], q2Bin )) ;
     cout << Form("Bkg events generated")  << endl;
     wsp_toy_bkg_temp->import(*toy_bkg);
      cout << Form("Bkg events imported")  << endl;
     wsp_toy_bkg_temp->writeToFile(filename_toy_bkg.c_str());
     cout << Form("Bkg events saved in file: %s",filename_toy_bkg.c_str())  << endl;
    }else{
     retrieveWorkspace( filename_toy_bkg, wsp_toy_bkg, Form("wsp_toy_bkg_%i_b%i.root",years[iy], q2Bin ));
     toy_bkg=(RooDataSet*)wsp_toy_bkg[iy]->data(Form("data_toy_bkg_%i_b%i",years[iy], q2Bin ));
    } 
    data[iy][itoy]->append(*toy_bkg);
//

    retrieveWorkspace( filename_sb_local, wsp_sb, "wsb");
    ...
    wksp->import(*bkg_ang_pdf, RecycleConflictNodes());
...
//    }      
    ...
    // import mass PDF from fits to the MCi 
    string filename_mc_mass;
    if (!localFiles) {
     filename_mc_mass = Form("/eos/cms/store/user/fiorendi/p5prime/massFits/noIP2D/xgbv8/results_fits_%i_fM_noIP2D_MCw_xgbv8.root",years[iy]);
    }else{
     filename_mc_mass = Form("results_fits_%i_fM.root",years[iy]);
    } 
...
// 
  ...
  TFile* fout = new TFile(("simFitResults4d/simFitResult_recoMC_fullAngularMass_sb_syst" + all_years + stat + Form("_b%i.root", q2Bin)).c_str(),"RECREATE");
...
//   fitResultsTree->Branch("usedPenalty",&usedPenalty);
  usedPenalty=false;
  std::cout<<Form("set usedPenalty=%d",usedPenalty)<<std::endl;
...
//     }
    ...
    RooSimultaneous* simPdf_forFit  = (RooSimultaneous* ) wksp->pdf("simPdf");
                     simPdf_penalty = (RooSimultaneous* ) wksp->pdf("simPdf_penalty");
...
//     }
    ...
    fitter->setNCPU(_SC_NPROCESSORS_CONF,_SC_NPROCESSORS_CONF/4,_SC_NPROCESSORS_CONF);
    vFitter.push_back(fitter);

    subTime.Start(true);
    cout<<"===== Starting fit ===== "<<endl;
    int status = fitter->fit();
    cout<<"===== Ending fit  ===== "<<endl;
    subTime.Stop();
    ...
    if (status==0) {
      ...
      std::cout<<"Fit status 0"<<std::endl;
...
//    fitResultsTree->Write();
        std::cout<<"Import combData&simPdf in RooWorkspace wsp_out\n"<<std::endl;
	wsp_out = new RooWorkspace("wsp_out","wsp_out");
        RooArgSet *params_fit      = (RooArgSet *)simPdf->getParameters(observables);
	wsp_out->import(*params_fit);
...
//     }
//   }
//   c[confIndex]->SaveAs( ("plotSimFit4d_d/simFitResult_recoMC_fullAngularMass_sb_syst" + plotString +  ".pdf").c_str() );
